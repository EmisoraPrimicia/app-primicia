...
const AppConfig = (props = {}) => {
    ...
    const _changeTheme = (theme, colorScheme) => {
        ...
        if (!themeLink) {
            // Si no existe, buscar el link con el tema base de primereact
            const links = document.querySelectorAll('link[rel="stylesheet"]');
            for (const link of links) {
                if (link.href.includes('primereact/resources/themes')) {
                    themeLink = link;
        ...
        // Crear un nuevo link para pre-cargar el tema sin flash
        const clonedLink = themeLink.cloneNode(true);
        clonedLink.href = newHref;
        clonedLink.id = 'theme-css-clone';
        ...
        clonedLink.addEventListener('error', () => {
            console.error(`Error al cargar tema: ${theme}`);
            clonedLink.remove();
        });
