    ...
    return (
        <li className={classNames({ 'layout-root-menuitem': props.root, 'active-menuitem': active })}>
            ...
            {(!item.to || item.items) && item.visible !== false ? (
                <a href={item.url} onClick={itemClick} className={classNames(item.class, 'p-ripple')} target={item.target} tabIndex={0}>
            ...
            {item.to && !item.items && item.visible !== false ? (
                <Link to={item.to} replace={item.replaceUrl} target={item.target} onClick={itemClick}
